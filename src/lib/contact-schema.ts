...
export type ContactValidationMessages = {
  name: string; email: string; phone: string; service: string; budget: string; details: string;
};

export const createContactSchema = (messages: ContactValidationMessages) => z.object({
  name: z.string().trim().min(1, messages.name).max(100),
  phone: z.string().trim().min(7, messages.phone).max(24).regex(/^[+\d][\d\s().-]{6,23}$/, messages.phone),
  company: z.string().trim().max(120).optional().default(''),
  email: z.string().trim().email(messages.email).max(160),
  services: z.array(z.enum(serviceIds)).min(1, messages.service).max(serviceIds.length),
  budget: z.enum(budgetIds, { message: messages.budget }),
  details: z.string().trim().min(20, messages.details).max(5000),
